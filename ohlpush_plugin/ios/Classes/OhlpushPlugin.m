...
@interface OhlpushPlugin() <FlutterStreamHandler>
...
@implementation OhlpushPlugin

+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    // 标准: MethodChannel 统一命名：com.mob.项目xx.功能
    NSObject<FlutterBinaryMessenger> *messenger =[registrar messenger];
    FlutterMethodChannel* channel = [FlutterMethodChannel methodChannelWithName:METHOD_CHANNEL binaryMessenger:messenger];
    OhlpushPlugin *instance = [[OhlpushPlugin alloc] init];
    [registrar addMethodCallDelegate:instance channel:channel];
    
    FlutterEventChannel *e_channel = [FlutterEventChannel eventChannelWithName:EVENT_CHANNEL binaryMessenger:messenger];
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult)result {
    if ([@"getPlatformVersion" isEqualToString:call.method]) {
        ...
        result([OHLPush sdkVersion]);
    } else if ([@"getRegistrationId" isEqualToString:call.method]) {
        [OHLPush getRegistrationID:^(NSString *registrationID, NSError *error) {
            if (error) {
        ...
        result(@([OHLPush isPushStopped]));
    } else if ([@"setAlias" isEqualToString:call.method]) {
        NSDictionary *arguments = (NSDictionary *)call.arguments;
        if (arguments
            && arguments[@"alias"]
            && [arguments[@"alias"] isKindOfClass:[NSString class]]) {
            [OHLPush setAlias:arguments[@"alias"] result:^(NSError *error) {
        ...
        [OHLPush getAliasWithResult:^(NSString *alias, NSError *error) {
            if (error) {
                result(@{@"res": @"", @"error": error.localizedDescription});
            } else {
        ...
        [OHLPush deleteAlias:^(NSError *error) {
            NSString *errorStr = error ? error.localizedDescription : @"";
            result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
        }];
        ...
        if (arguments && arguments[@"tags"]) {
            [OHLPush addTags:arguments[@"tags"] result:^(NSError *error) {
                NSString *errorStr = error ? error.localizedDescription : @"";
                result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
            }];
        } else {
        ...
        [OHLPush getTagsWithResult:^(NSArray *tags, NSError *error) {
            if (error) {
                result(@{@"res": @"", @"error": error.localizedDescription});
            } else {
        ...
        NSDictionary *arguments = (NSDictionary *)call.arguments;
        if (arguments && arguments[@"tags"]) {
            [OHLPush deleteTags:arguments[@"tags"] result:^(NSError *error) {
                NSString *errorStr = error ? error.localizedDescription : @"";
                result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
            }];
        } else {
        ...
        [OHLPush cleanAllTags:^(NSError *error) {
            NSString *errorStr = error ? error.localizedDescription : @"";
            result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
        }];
        ...
        if (arguments && arguments[@"localNotification"]) {
            ...
            OHLPushNotification *noti = [[OHLPushNotification alloc] init];
            ...
            OHLPushNotificationTrigger *trigger = [OHLPushNotificationTrigger new];
            ...
            OHLPushNotificationRequest *request = [OHLPushNotificationRequest new];
            request.requestIdentifier = identifier;
            request.content = noti;
            request.trigger = trigger;
            
            [OHLPush addLocalNotification:request result:^(id res, NSError *error) {
                NSString *errorStr = error ? error.localizedDescription : @"";
                result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
            }];
        }
        ...
        if (arguments && arguments[@"phoneNum"]) {
            [OHLPush bindPhoneNum:arguments[@"phoneNum"] result:^(NSError *error) {
                NSString *errorStr = error ? error.localizedDescription : @"";
                result(@{@"res": error ? @"failed": @"success", @"error": errorStr});
            }];
        } else {
        ...
        if (arguments) {
            ...
            [OHLPush sendMessageWithMessageType:type
                                        content:content
                                          space:space
                                          sound:sound
        ...
        if (userLang) {
            [OHLPush setUserLanguage:userLang];
        }
...
- (void)didReceiveMessage:(NSNotification *)notification {
    if ([notification.object isKindOfClass:[OHLPushMessage class]]) {
        OHLPushMessage *message = (OHLPushMessage *)notification.object;
        NSMutableDictionary *resultDict = [NSMutableDictionary dictionary];
        NSMutableDictionary *reslut = [NSMutableDictionary dictionary];
        switch (message.messageType) {
            case OHLPushMessageTypeCustom: {// 自定义消息
                ...
                break;
            case OHLPushMessageTypeAPNs: {// APNs 回调
                if (message.notification.userInfo) {
                    [reslut setObject:message.notification.userInfo forKey:@"extrasMap"];
                }
                ...
                break;
            case OHLPushMessageTypeLocal: { // 本地通知回调
                if (message.notification.userInfo) {
                    [reslut setObject:message.notification.userInfo forKey:@"extrasMap"];
                }
            ...
            case OHLPushMessageTypeClicked: {
                if (message.notification.userInfo) {
                    [reslut setObject:message.notification.userInfo forKey:@"extrasMap"];
                }
